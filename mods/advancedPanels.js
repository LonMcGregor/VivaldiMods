...
(function advancedPanels() {
  ...
  const l10n = {
    ...
    it: {
      ...
      delete_number_sessions:
        "Sei sicuro di voler eliminare $N sessioni selezionate?",
    ...
    de: {
      ...
      delete_number_sessions:
        "Wollen Sie die $N ausgewählten Sitzungen wirklich löschen?",
    ...
    nl: {
      ...
      delete_number_sessions:
        "Weet u zeker dat u $N geselecteerde sessies wilt verwijderen?",
    ...
    pt_br: {
      ...
      delete_button: "Deletar esta sessão",
      delete_prompt: "Você tem certeza que deseja deletar $T?",
      delete_number_sessions:
        "Você tem certeza que deseja deletar $N sessões selecionadas?",
      delete_confirm: "⚠ Sim, delete",
      delete_abscond: "Não, não quero.",
      time_created_label: "Criado <time></time>",
      ...
      module: function () {
        ...
        function getSelectedSessionNames() {
          const selections = Array.from(
            document.querySelectorAll("#sessions_lonm li.selected")
          );
        ...
        function listItemClick(e) {
          ...
          if (e.ctrlKey) {
            ...
            const oldselect = document.querySelectorAll(
          ...
          document
            .querySelector("#sessions_lonm .confirm")
        ...
        function isButton(target) {
          ...
          return (
            (tag === "button" && target.className === "delete") ||
            (tag === "svg" && target.parentElement.className === "delete")
          );
        }
        ...
        function newSessionClick(e) {
          let name = document.querySelector(
            "#sessions_lonm .newSession input.session-name"
          ).value;
          const windows = document.querySelector(
            "#sessions_lonm .newSession input.all-windows"
          ).checked;
          const selectedTabs = document.querySelector(
            "#sessions_lonm .newSession input.selected-tabs"
          ).checked;
          ...
          vivaldi.windowPrivate.getCurrentId((window) => {
            ...
            if (selectedTabs && markedTabs && markedTabs.length > 0) {
              options.ids = Array.from(markedTabs).map((tab) =>
                Number(tab.id.replace("tab-", ""))
              );
            }
        ...
        function sortOrderChange(e) {
          document
            .querySelectorAll("#sessions_lonm .sortselector-button")
            .forEach((el) => {
        ...
        function deleteClick(e) {
          ...
          if (selectedSessions.length === 1) {
            const delete_t = l10n.delete_prompt.replace(
              "$T",
              selectedSessions[0]
            );
            confirmMsg(delete_t);
          } else {
            const delete_n = l10n.delete_number_sessions.replace(
              "$N",
        ...
        function confirmMsg(msg) {
          ...
          document
            .querySelector("#sessions_lonm .modal-container")
            .classList.add("show");
        }
        ...
        function openInCurrentWindowClick(e) {
          ...
          selections.forEach((item) => {
            vivaldi.sessionsPrivate.open(item, { openInNewWindow: false });
          });
        }
        ...
        function oneInNewWindowClick(e) {
          const selections = getSelectedSessionNames();
          selections.forEach((item) => {
            vivaldi.sessionsPrivate.open(item, { openInNewWindow: true });
          });
        }
        ...
        function createListItem(session) {
          const template = document.querySelector(
            "#sessions_lonm template.session_item"
          );
          ...
          el.querySelector(".open_new").addEventListener(
            "click",
            oneInNewWindowClick
          );
          el.querySelector(".open_current").addEventListener(
            "click",
            openInCurrentWindowClick
          );
          el.querySelector(".delete").addEventListener("click", deleteClick);
          el.querySelector("li").setAttribute(
            "data-session-name",
            session.name
          );
          return el;
        ...
        function sortSessions(sessions) {
          const sortRule = document.querySelector(
            "#sessions_lonm .sortselector-dropdown"
          ).value;
          const sortDescending = document.querySelector(
            "#sessions_lonm .direction-descending.selected"
          );
          if (sortRule === "visitTime" && sortDescending) {
            sessions.sort((a, b) => {
              return a.createDateJS - b.createDateJS;
            });
          } else if (sortRule === "visitTime" && !sortDescending) {
            sessions.sort((a, b) => {
              return b.createDateJS - a.createDateJS;
            });
        ...
        function updateList() {
          ...
          vivaldi.sessionsPrivate.getAll((items) => {
            ...
            document
              .querySelector("#sessions_lonm .sessionslist")
              .appendChild(newList);
          });
        }
        ...
        function onInit() {
          ...
          document
            .querySelector("#sessions_lonm .sortselector-dropdown")
            .addEventListener("change", sortMethodChange);
          document
            .querySelector("#sessions_lonm .confirm .yes")
            .addEventListener("click", deleteConfirmClick);
          document
            .querySelector("#sessions_lonm .confirm .no")
            .addEventListener("click", deleteCancelClick);
          document
            .querySelector("#sessions_lonm .newSession .add-session")
            .addEventListener("click", newSessionClick);
        }
  ...
  function observeUIState() {
    ...
    UI_STATE_OBSERVER.observe(
      document.querySelector("#panels-container"),
      classInit
    );
  }

  const PANEL_STACK_CREATION_OBSERVER = new MutationObserver(
    (records, observer) => {
      observer.disconnect();
      listenForNewPanelsAndConvertIfNecessary();
    }
  ...
  function advancedPanelCreated(node, panel, panelID) {
    ...
    node.id = panelID;
    panel.module().onInit();
    ADVANCED_PANEL_ACTIVATION.observe(node, {
      attributes: true,
      attributeFilter: ["class"],
    });
  ...
  function convertWebPanelButtonstoAdvancedPanelButtons() {
    for (const key in CUSTOM_PANELS) {
      ...
      let switchBtn = document.querySelector(
        `#switch button[title~="${panel.url}"`
      );
      if (!switchBtn) {
        switchBtn = document.querySelector(
          `#switch button[advancedPanelSwitch="${key}"`
        );
        if (!switchBtn) {
