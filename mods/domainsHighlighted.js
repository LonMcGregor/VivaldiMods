...
(function(){
    ...
    function domainsHighlighted(addressBarElement){
        ...
        function maskSimplifiedAddress(newText){
            const positionStart = newText.indexOf("/");
            if(isNaN(positionStart) || positionStart<=0){
                return clearMask();
            }
            const widthToSlash = checkTextWidth(newText.substring(0,positionStart));
            const mask = createWebkitMask(0, widthToSlash+10);
            applyMask(mask);
        }
